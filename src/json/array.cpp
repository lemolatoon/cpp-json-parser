      ...
      choices(
        parsers::map(
          ...
          [&](auto elems) {
            ...
            return std::make_pair(std::string_view{input.data(), length - 1 /* remove of trailing comma */}, std::move(elems));
          }
        ),
        parsers::map(
          whitespace,
          [&](auto ws) {
            return std::make_pair(ws.original(), std::move(std::vector<Value>{}));
          }
        )
      ),
      parsers::character(']')
